...
// and carries a static byte size when known. Combinators (`arr`, `struct`,
// `reserved`, `nulStr`, `fixedStr`) build larger codecs from smaller ones;
// `encode` / `decode` / `decodePadded` / `sizeOf` are top-level helpers.
...
// Primitive codec namespace.  All entries are little-endian.
export const Codec = {
...
export function reserved(n: number): BinCodec<undefined> {
  return {
    size: n,
...
export function arr<T>(codec: BinCodec<T>, n: number): BinCodec<T[]> {
...
export function nulStr(maxBytes: number): BinCodec<string> {
  return {
    size: maxBytes,
...
export function fixedStr(n: number): BinCodec<string> {
  return {
    size: n,
...
export function struct<F extends Record<string, BinCodec<any>>>(fields: F): BinCodec<StructValue<F>> {
...
// Re-exported for callers building their own struct types without inference.
export type { FieldsOf, StructValue };

// Top-level encode / decode helpers.
// Spin up a single-shot reader/writer for any fixed-size codec, so simple
// "convert one value to bytes / read bytes back as one value" call sites
// don't have to construct BinReader/BinWriter explicitly.

// Encode a value with a fixed-size codec.  Throws if the codec is variable-width
export function encode<T>(codec: BinCodec<T>, value: T): Uint8Array {
...
export function decode<T>(codec: BinCodec<T>, bytes: Uint8Array): T {
...
export function decodePadded<T>(codec: BinCodec<T>, bytes: Uint8Array): T {
...
export function sizeOf<T>(codec: BinCodec<T>): number {
  if (codec.size === undefined) {
    throw new Error('binCodec.sizeOf: codec has no fixed size');
  }
  return codec.size;
}

...
describe('enqueue (bulk immediate)', () => {
  ...
  it('applies the mutator optimistically and sends one bulk write', async () => {
    ...
    enqueue({ control: 'eqFilter', mutate: (s) => { s.masterVolumeDb = -12; } });
  ...
  it('coalesces: commits during an in-flight send trigger exactly one more send', async () => {
    ...
    enqueue({ control: 'eqFilter', mutate: (s) => { s.masterVolumeDb = -1; } });
    enqueue({ control: 'eqFilter', mutate: (s) => { s.masterVolumeDb = -2; } });
    enqueue({ control: 'eqFilter', mutate: (s) => { s.masterVolumeDb = -3; } });
    expect(sends).toBe(1);
  ...
  it('on send failure sets error status and leaves the lane usable (not wedged)', async () => {
    ...
    enqueue({ control: 'eqFilter', mutate: (s) => { s.masterVolumeDb = -5; } });
    ...
    enqueue({ control: 'eqFilter', mutate: (s) => { s.masterVolumeDb = -6; } });
    expect(sends).toBe(2);
  });
  ...
  it('settle is silent (does not advance lastSentRev) when generation changed mid-flight', async () => {
    ...
    let sends = 0;
    bindBulkDevice(() => { sends += 1; return new Promise<void>((res) => { resolveSend = res; }); });
    enqueue({ control: 'eqFilter', mutate: (s) => { s.masterVolumeDb = -7; } });
    expect(sends).toBe(1);
  ...
  it('cancel clears the bulk lane so it is idle and not wedged', () => {
    ...
    enqueue({ control: 'eqFilter', mutate: (s) => { s.masterVolumeDb = -4; } });
    expect(sends).toBe(1);
    ...
    enqueue({ control: 'eqFilter', mutate: (s) => { s.masterVolumeDb = -8; } });
    expect(sends).toBe(2);
  });
  ...
  it('a detached stale send cannot clear or duplicate a newer in-flight bulk send', async () => {
    ...
    enqueue({ control: 'eqFilter', mutate: (s) => { s.masterVolumeDb = -4; } });   // send A parks in flight
    expect(sends).toBe(1);
    expect(isInFlight.current).toBe(true);

    cancelWrites();                              // detaches A, bumps generation
    enqueue({ control: 'eqFilter', mutate: (s) => { s.masterVolumeDb = -8; } });   // send B parks in flight
    expect(sends).toBe(2);
...
describe('resync guard sees the bulk lane (Finding 1)', () => {
  ...
  it('a trailing resync does not clobber an in-flight bulk edit', async () => {
    let resolveSend!: () => void;
    bindBulkDevice(() => new Promise<void>((res) => { resolveSend = res; }));
    enqueue({ control: 'eqFilter', mutate: (s) => { s.masterVolumeDb = -12; } });   // optimistic; send parked in flight
    expect(dsp.draft?.masterVolumeDb).toBe(-12);
    scheduleResync();
